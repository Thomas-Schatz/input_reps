...
INDTYPE = np.intp_t # array index type
...
cpdef _dtw(IND_t N, IND_t M, CTYPE_t[:,:] dist_array, bool normalized, bool return_path):
    ...
    if normalized:
        final_cost /= path_len
    if return_path:
        cdef IND_t[:,:] path = np.empty((path_len, 2), dtype=INDTYPE)
        cdef IND_t k
        i = N-1
        j = M-1
        k = path_len
        ...
        if i == 0:
            # should have k == j
            path[:k, 0] = np.zeros((k,), dtype=INDTYPE)
            path[:k, 1] = np.arange(k, dtype=INDTYPE)
        if j == 0:
            # should have k == i
            path[:k, 1] = np.zeros((k,), dtype=INDTYPE)
            path[:k, 0] = np.arange(k, dtype=INDTYPE)
